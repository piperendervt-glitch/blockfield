...
namespace BlockField.SimCore.Fluid
{
    ...
    public struct JellyParams
    {
        ...
        /// <summary>
        /// **沈降速度**（遊泳速度に対する比）。世界法則（jelly_2 追記10）。
        ///
        /// 体の密度と水の密度の差は**種の性質**で、個体が調整するものではない。
        /// したがって遺伝子ではなく設計値。絶対値を直接置かず比で置くのは、
        /// **「拍動と沈降のどちらが勝つか」という関係を設計している**ことを
        /// 明示するため。
        ///
        /// 【なぜ要るか】沈降が無いと、傾いても旋回しても「何のために」が無い。
        /// 拍動を止めれば沈むなら、**拍動は沈降に抗う努力**になる。
        /// 実機で「重力がないため自然に見えない」と観察された（2026-08-16）。
        ///
        /// 【既定 1.10 は実測でなく実機の印象から選んだ】1 を超えるので
        /// **拍動しても沈む**（追記11 の掃引で不合格側と分かっている領域）。
        /// それでも「最もバランスが取れて見える」という体験的判断で採った。
        /// 判定は「沈まない」から「沈降を遅くする」へ弱めてある（追記12 M-K2k′）。
        /// 世界定数なので設計してよい側だが、**体験から決めた事実を消さない**
        ///
        /// Phase D の浮力項とは別物。あちらは**水**が動く話、こちらは**体**が沈む話。
        /// </summary>
        ...
        public static JellyParams Default => new JellyParams
        {
            ...
            JetModel = false,
            Pacemaker = true,
            // 【0.25 から 0.90 へ変更】掃引で釣り合いが比 ≈ 1.0 と分かった。
            // 0.25 では拍動中に 0.0299 m/s で上昇し続け、20秒で 0.6m 上がる
            // （2m の部屋なら1分で天井）。「拍動＝沈まないための努力」は
            // 比が 1 の近くでしか成立せず、それ以下では単なる上向き推進である。
            // 0.90 は「わずかに拍動が勝つ」点（正味 +0.0039 m/s）。
            // 1.0 ちょうどにしないのは、正味 0 だと鉛直に一切動かなくなるため。
            // 経緯は prereg jelly_2 追記11
            SinkRatio = 1.10f,
