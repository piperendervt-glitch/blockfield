namespace BlockField.SimCore.Ecology
{
    ...
    public static class EcologyStats
    {
        ...
        /// | コロニー(豚) | 0.040 | 0.210 | 1.000 | 0.20 | 254 |
        /// | コロニー(羊) | 0.042 | 0.234 | 1.000 | 0.20 | 257 |
        /// | コロニー(狼) | 0.039 | 0.151 | 1.000 | 0.20 | 154 |
        ///
        /// 【コロニー場の基準値は2度取り直している】この場は段ごとに分布が変わった:
        ///
        /// | 段階 | 場の中身 | 90%点 | 基準値 |
        /// |---|---|---|---|
        /// | 4a (K1) | 繁殖の痕跡だけ。稀に立つ濃い点 | 0.085〜0.096 | 0.10 |
        /// | 4a 追補 | ＋滞在の痕跡。広く薄い下地 | 0.047〜0.050 | 0.05 |
        /// | 4c (K4) | ＋群れ行動。個体が留まって濃くなる | 0.151〜0.234 | **0.20** |
        ///
        /// 4c で濃くなったのは、群れた個体が同じ場所に留まり続けて
        /// 滞在の書き込みが積み上がるため（上位10%セルへの集中度も 0.30 → 0.76）。
        /// **場を読む機構を足すと場そのものの分布が変わる**という、
        /// この場に固有の事情がよく出ている例である。
        ///
        ...
        public static float FieldDisplayScale(string fieldName) => fieldName switch
        {
            ...
            // コロニー場 (Demo 8 第4段。0.10 → 0.05 → 0.20 と2度取り直している)。
            // 4c で群れ行動が入り、個体が同じ場所に留まるようになったので
            // 場が一気に濃くなった（90%点 0.047〜0.050 → 0.151〜0.234、
            // 上位10%セルへの集中度も 0.30 → 0.76）。
            // 恐怖場・獲物場と同じ 0.20 が実測分布の中心にあたる
            "colony-pig" or "colony-sheep" or "colony-wolf" => 0.20f,
