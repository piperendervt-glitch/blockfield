namespace BlockField.SimCore.Ecology
{
    ...
    public static class EcologyStats
    {
        ...
        /// | コロニー(豚) | 0.034 | 0.085 | 0.934 | 0.10 | 10 |
        /// | コロニー(羊) | 0.036 | 0.088 | 0.689 | 0.10 | 0 |
        /// | コロニー(狼) | 0.034 | 0.096 | 0.650 | 0.10 | 0 |
        ///
        /// 【コロニー場が桁違いに薄い理由 (Demo 8 第4段 K1)】繁殖は 1000ティックあたり
        /// 7.25 回しか起きず、しかも種で割られる。48シードのうち痕跡が1セルも
        /// 立たないシードが 豚 11 / 羊 28 / 狼 33 ある。値の分布そのものは
        /// 死の場とほぼ同じ（90%点 0.085〜0.096 対 0.089）なので基準値は死の場と
        /// 揃えてあるが、**分布を測るには多数のシードを合算する必要がある**。
        /// これが <see cref="DisplayScaleMatchesDistribution"/> を使うテストが
        /// 1シードでなく複数シードを合算する理由である。
        ///
        ...
        public static float FieldDisplayScale(string fieldName) => fieldName switch
        {
            ...
            // コロニー場 (Demo 8 第4段 K1)。deposit 1.0 / τ≈400 / 拡散0.02 は
            // 死の場とほぼ同じ設定なので、値の分布も死の場と揃う
            // （90%点 0.085〜0.096 対 0.089）。違うのは頻度＝痕跡の数だけで、
            // 濃さの正規化は死の場と同じでよい
            "colony-pig" or "colony-sheep" or "colony-wolf" => 0.10f,
        ...
        ///
        /// 【平均が桁違いに小さいことに意味がある】繁殖は 1000ティックあたり 7.25 回で、
        /// 死（60回）の 1/8 しか起きない。したがってこの平均は死の場の1桁下になるのが
        /// 正常であり、**死の場と同じ桁になったら書き込みが多すぎる**（繁殖が
        /// 増えたか、書き込み条件を間違えた）という監視の意味を持つ。
