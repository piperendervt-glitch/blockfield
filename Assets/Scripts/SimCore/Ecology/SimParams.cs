namespace BlockField.SimCore.Ecology
{
    ...
    public struct SimParams
    {
        ...
        /// <summary>
        /// ワールド生成時に適性セルへ入れておく草の量（Demo 8.5、暫定対処）。
        /// 実際の初期値は `suitability × この値`（適性0のセルは0のまま）。
        ///
        /// 【なぜ要るか】ロジスティック成長は立ち上がりが緩やかで、
        /// 0から始めると平衡（適性セルあたり約0.135）に達するまで
        /// 1,500ティックほどかかる。**実機セッションは5分＝約400ティック**
        /// （CLAUDE.md）しかなく、その時点の適性セルあたりは 0.070 で
        /// 草の表示閾値（下限0.16）に届かない。実機で草が1つも見えず
        /// M1/M5 が判定不能になった原因のひとつ。
        ///
        /// 平衡付近から始めれば5分のセッションでも判定できる。
        /// 0.13 は実測の平衡値（ロジスティックの釣り合いと摂食のバランスで
        /// 決まる）。
        ///
        /// **RNG を消費しない**（suitability からの決定的な計算のみ）ので
        /// 決定論は保たれる。
        ///
        /// これは**暫定対処**である。正式には「セッション長で判定できる
        /// 表示閾値」か「実機の時間スケール」のどちらかを見直すべきで、
        /// その判断は帰宅後に行う。
        /// </summary>
