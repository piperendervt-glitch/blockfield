namespace BlockField.SimCore.Ecology
{
    ...
    public struct SimParams
    {
        ...
        /// <summary>
        /// 変異率: 繁殖時、子の重みの**成分ごと**に変異が起きる確率
        /// (Demo 8 第4.5段 K1)。**既定 0 = 変異なし**で、現行の挙動と完全に同一。
        /// E1/E2 の実験条件として有効化する。
        ///
        /// 【0 のときは乱数を1個も引かない】<see cref="mutationSigma"/> と
        /// どちらかが 0 なら変異処理そのものを飛ばす。飛ばさずに
        /// 「引いて捨てる」形にすると乱数列がずれ、変異を入れる前の世界と
        /// ContentHash が一致しなくなる（M 判定の前提が壊れる）。
        /// </summary>
        public float mutationRate;

        /// <summary>
        /// 変異幅: 加えるガウスノイズの標準偏差 (Demo 8 第4.5段 K1)。**既定 0**。
        ///
        /// 初期候補は現行重みのスケールの 5〜10%（w_colony=2.4 に対し 0.1〜0.25）。
        /// 値は E1/E2 の前に掃引して決める（prereg「変異パラメータの決定」）。
        ///
        /// **上下限は設けていない。** 重みが負に振れれば「自種を避ける」、
        /// 大きく振れれば「群れに強く引かれる」という意味になり、
        /// どちらも探索する価値のある領域である。クリップすると
        /// 分布の端が人工的に溜まり、浮動の観察（E2）が歪む。
        /// </summary>
        ...
        ///
        /// 【4c で 12 → 80 に取り直した — K3 と K4 は同じ場で結ばれている】
        /// k=12 は **w_colony=0（群れ無し）の場の分布を前提に**決めた値だった。
        /// 4c で群れ行動を入れると、群れた場所のコロニー場が濃くなる。
        /// この場は繁殖確率も決めているので、
        ///   群れる → 場が濃くなる → 繁殖確率が上がる → 個体が増える → さらに濃くなる
        /// という正のフィードバックが立つ。実測では出生が 5.6 → 34.8/1000t（6倍）に
        /// 暴走し、局所の餌が枯れて**草食ギルドが 2〜10/48 のシードで全滅**した
        /// （prereg リスク欄の「過密の正フィードバック」がそのまま起きた）。
        ///
        /// w=1.6 固定で k だけを動かした分離計測が原因を確定させている:
        ///
        /// | k | 出生/1000t | ギルド絶滅 | 群れ（有意な検定の数） |
        /// |---|---:|---:|---|
        /// | 12 | 34.83 | 2/48 | 6/6 |
        /// | 40 | 18.17 | **0/48** | 6/6 |
        /// | 80 | 8.63 | **0/48** | 5/6 |
        ///
        /// **k を上げると群れは残ったまま崩壊だけが消える。** つまり過密崩壊の原因は
        /// 群れそのものではなく、群れが繁殖を押し上げたことだった。
        /// 4c の既定 (w=2.4) に合わせて k=80 とし、
        /// 出生 14.5/1000t・ギルド絶滅 0/48 に収めてある。
        /// </summary>
        public float colonyBreedK;

        /// <summary>
        /// 移動の重み: **自種のコロニー場**へどれだけ引かれるか (w_colony、
        /// Demo 8 第4段 K4)。群れ行動の本体。
        ///
        /// 【満腹のときだけ効く — 実測で決めた】この重みが入るのは
        /// <see cref="EntityWeights.WanderingFor"/>（満腹時の徘徊）**だけ**で、
        /// <see cref="EntityWeights.ForagingFor"/>（採餌・狩り）には入らない。
        ///
        /// 採餌中も群れに引かれる形を試したところ、草食獣が餌を探すのをやめて
        /// 固まり続け、**獲物場が一点に集中して狼が獲物を見つけられなくなった**
        /// （獲物場の集中度 0.45 → 0.71〜0.82、捕食率 1.07 → 0.09、
        /// 狼が 5/5 のシードで全滅）。匂いで狩る捕食者にとって、
        /// 獲物が固まることは「探索面積が消える」ことを意味する。
        /// 満腹時だけ群れる形にすると、空腹の個体は散って餌を探すので
        /// 匂いが世界に残り、捕食が成立したまま群れが出る。
        ///
        /// 【値の決め方 — 48シード掃引】0.15〜0.25 では群れが立たず（符号検定が
        /// 偶然の範囲）、0.3 以上で群れ始めると同時に草食ギルドの全滅が現れる。
        /// これは群れのせいではなく繁殖の暴走のせいで、
        /// <see cref="colonyBreedK"/> を取り直すと崩壊だけが消えた（そちらの表を参照）。
        /// k=80 と組み合わせた w=2.4 で、M4 の6検定すべてが有意・
        /// ギルド絶滅 0/48・狼の全滅が対照と同じ 1/48 に収まる。
        ///
        ...
        public static SimParams Default => new SimParams
        {
            ...
            colonyDecay = 0.0025f,
            // 群れ行動の重み (Demo 8 第4段 K4)。48シード掃引で決めた
            colonySelfWeight = 2.4f,
