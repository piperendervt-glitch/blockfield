namespace BlockField.SimCore.Ecology
{
    ...
    public struct SimParams
    {
        ...
        /// 摂食対象を探すときの閾値。
        ///
        /// 【0.70 は段階1〜2の中間状態専用の暫定値】
        /// 中間状態では植生場が「草そのもの」ではなく
        /// **植物の周囲ににじんだ痕跡**である。低い閾値にすると餌場が
        /// 植物本体よりはるかに広くなる（実測: 閾値0.05 で 1,681セル対
        /// 植物220本＝7.6倍）。実際それで餓死率が基準の 1/4.4 に落ちた。
        ///
        /// 移行前の餌場は実質的に **veg ≧ 0.76 のセル**だった
        /// （植物のあるセルの植生場は 0.76〜0.98、中央値 0.939）。
        /// 0.70 はこれとほぼ一致する（植物の1.25倍）。
        ///
        /// **段階3で植生場が「草そのもの」になった時点で 0.05 付近へ戻すこと。**
        /// そのときは薄い草も食べられるのが正しく、
        /// 表示（K3）の最低段階 0.2 より低いほうが
        /// 「見えるより先に食べ尽くす」で自然になる。
        /// </summary>
        ...
        public static SimParams Default => new SimParams
        {
            ...
            grazeBite = 0.5f,
            grazeRecovery = 2f,
            grazeThreshold = 0.70f, // 段階1〜2の暫定値。段階3で 0.05 付近へ戻す
            vegetationGrowth = 0.05f,
            fearDeposit = 0.5f,
            fearDiffuse = 0.1f,
            fearDecay = 0.03f,
