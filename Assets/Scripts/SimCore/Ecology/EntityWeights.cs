...
namespace BlockField.SimCore.Ecology
{
    ...
    public struct EntityWeights : IEquatable<EntityWeights>
    {
        ...
        public static EntityWeights ForagingFor(EntityKind kind, SimParams p)
        {
            ...
            // **群れ重みは採餌時には入れない** (Demo 8 第4段 K4、実測で決めた)。
            // 採餌中も群れに引かれると草食獣が餌を探すのをやめて固まり続け、
            // 獲物場が一点に集中する。すると匂いで狩る狼は群れから離れた瞬間に
            // 勾配を失い、捕食率が 1.07 → 0.09 に崩れて 5/5 のシードで全滅した。
            // 満腹のときだけ群れれば、空腹の個体は散って餌を探すので
            // 匂いが世界に残り、捕食が成立したまま群れが出る。
            // 詳細は SimParams.colonySelfWeight のコメントを参照
            return w;
        }
