...
namespace BlockField
{
    ...
    public sealed class FieldOverlayView : MonoBehaviour
    {
        ...
        void Rebuild()
        {
            ...
            {
                // 【植生場だけ意味と違う色を使う】他の場は意味に沿った色
                // （恐怖=赤 / 獲物=青 / 死=マゼンタ / 踏み荒らし=茶）だが、
                // 植生場を緑にすると**地形の草ブロックと同系色**になり、
                // 図と地が分離せず濃淡が読めない（エディタ確認で報告された）。
                // 茶（踏み荒らし）が緑の地形の上でよく見えるのと同じ理屈で、
                // 地形に無い色相＝シアンを充てる。
                // 「植生場＝緑」という直感からは外れるので、凡例で補う
                Layer.Vegetation => new Color32(60, 230, 220, 255),
