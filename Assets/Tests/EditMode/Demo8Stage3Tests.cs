...
namespace BlockField.Tests.EditMode
{
    ...
    public class Demo8Stage3Tests
    {
        ...
        public void M2_PlantsAreScarceOnTrampledCells()
        {
            ...
            // 【基準値が動いた (2026-08-11、暫定)】Demo 8.5 で草の初期値
            // （initialVegetation = 0.13）を入れたため、踏み跡のセルにも
            // 最初から草があり比が上がった（0.192 → 0.524）。
            // 初期値は**5分の実機セッションで草を判定可能にするための暫定対処**であり、
            // 正式な基準改定は帰宅後に行う（demo85_checklist.md に記録）。
            // ここでは新しい実測値に合わせて閾値を緩めるが、
            // 「踏み跡のほうが草が薄い」という主張自体は維持する
