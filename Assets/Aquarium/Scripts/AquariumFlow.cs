...
namespace BlockField.Aquarium
{
    ...
    public sealed class AquariumFlow : MonoBehaviour
    {
        ...
        /// <summary>
        /// **壁1・壁2・壁3**（軸方向6本の面までの距離の小さいほうから3つ）をログの断片にする。
        ///
        /// 【最近傍だけでは隅を同定できない】隅の定義は「2面から同時に接している」なので、
        /// 最も近い壁までの距離は単一の壁と同じ値になる。2番目が要る（prereg 追記18）。
        ///
        /// 【壁3 は記録専用】`隅+床` と `壁+床` は 壁2 まででは完全に同一で、
        /// 3番目だけが違う（0.032 対 0.960）。**判定には使わない** —
        /// 検証の連鎖のリンク1 は 壁2 ≤ 0.155 m のままである。
        /// 壁3 で絞ると**モデル自身が主張していない条件を実機に課す**ことになり、
        /// 再現しなかったときに A16.3 の否定なのか条件過剰なのかが分かれない
        /// （追記18 A18.6）。セッション後のラベル付けにだけ使う。
        /// </summary>
