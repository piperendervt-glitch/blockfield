...
| `--out DIR` | `runs/日時` | 出力先 |
| `--control` | off | 同一シードで対照（群れ重み `w_colony` のみ0）を並走させる。**所要時間はおよそ2倍** |
...
### 並走する対照（`--control`、Demo 8 第4段 K5）

`--conditions` に条件を並べると、条件ごとに**別々のシード集合の平均**を
比べることになる。群れ指標のように分散の大きい量では、その差が機構の効果なのか
シードの引きなのか分からない。`--control` は**同一シードで対にして**
対照を走らせ、シードごとの差を取る（地形と初期配置が相殺される＝対応のある比較）。

- 対照の定義は現在1種類: **群れ重み `w_colony` のみ 0**。他は本条件と完全に同一
- 対照は**別の World・別の乱数**で走るので、本条件の進行には一切影響しない
  （`--control` の有無でハッシュが変わらないことを 48シードで確認済み）
- 4c で `w_colony` の既定が 2.4 になったので、**対照は本条件と別の世界になる**
  （K5 の時点では既定が0で、差が全て0になるのが正常だった）。
  群れ指標の差が 4c の M4 判定そのものになる
- 将来の拡張として全重み0のランダム歩行対照（roadmap v9「地図性能の計測」）を
  `Condition.AsRandomWalkControl()` に器だけ用意してある（未使用）
...
| `summary.json` | 条件ごとの集計＋シードごとの ContentHash（決定論の追跡用）＋群れ指標（`flock` / `flockBySeed`） |
| `population.csv` | `condition,seed,tick,plants,herbivores,wolves`（10ティック間隔） |
| `images/*.png` | 地形俯瞰と場のヒートマップ |

#### 群れ指標（`flock`、Demo 8 第4段 K5）

4c の M4「群れの創発」を測る。**種別（sheep/pig/wolf）に分けて**出す。

| 指標 | 意味 | 群れると |
|---|---|---|
| `neighborMean` | 半径3セル内の同種個体数の平均（warmup 以降の時間平均） | 大きくなる |
| `pairDistanceMedian` | 全同種ペアの水平距離の中央値（同上） | 小さくなる |
| `colonyConcentrationTop10` | コロニー場の上位10%セルが総量に占める割合（最終時点） | 大きくなる（**記録項目**） |

- 一様分布の目安: ペア距離 ≈ 0.5214 × 一辺（50セルなら約26.1）、集中度 0.1
- 集中度を記録項目に留めるのは、出生位置の +X バイアス（4a で判明）にも
  反応するため。**判定は近傍数とペア距離で行う**（バイアスに不変な量）
- その種が居ないティック／シードは標本から外してある。0 を入れると
  「絶滅していた」が「群れていなかった」として平均に混ざるため。
  実際に何シードで測れたかは `flock.seeds` にある
- シードごとの値は `flockBySeed` にあり、対照を並走させていれば
  同じ行に対照の値も入るので、**対応のある検定がそのまま組める**
- 計算は O(A²)（A≈21）だが、`msPer1000Ticks`（Demo 8.5 M1 の基準値）を
  汚さないよう**計測時間から外してある**
